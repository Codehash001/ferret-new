...
const ChatbotPage: React.FC<ChatbotPageProps> = ({ params }) => {
  const [allchatbots, setAllChatbots] = useState<any[]>([]);
  const [chatbotExists, setChatbotExists] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    const fetchchatbots = async () => {
      try {
        ...
        setAllChatbots(data);
        setChatbotExists(data.some((chatbot) => chatbot.name === params.chatbot));
      } catch (error) {
